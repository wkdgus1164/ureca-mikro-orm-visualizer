 ...
 * 좌측 패널 (탭 구조)
 *
 * Entities 탭: 노드 목록 (기존 NodeListPanel)
 * AI Chat 탭: AI 채팅 인터페이스
 */

import { useState, useMemo, useCallback } from "react"
import { useReactFlow } from "@xyflow/react"
import { Box, MessageSquare, List, FileCode } from "lucide-react"
...
export function LeftPanel({ className }: LeftPanelProps) {
  ...
  return (
    ...
    >
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "entities" | "chat")}
        className="flex flex-col h-full"
      >
        <TabsList className="mx-2 mt-2 h-9 grid grid-cols-2">
